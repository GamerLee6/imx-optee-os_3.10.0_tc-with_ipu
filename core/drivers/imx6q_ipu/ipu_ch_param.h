#ifndef IPU_PARAM_RAM_H
#define IPU_RARAM_RAM_H
...
#define ipu_ch_param_set_field_io(base, w, bit, size, v) { \
	...
	reg_offset += i; \
	temp = io_read32((vaddr_t)(base) + reg_offset); \
	temp |= (v) << off; \
	io_write32((vaddr_t)(base) + reg_offset, temp); \
	if (((bit)+(size)-1)/32 > i) { \
		reg_offset++; \
		temp = io_read32((vaddr_t)(base) + reg_offset); \
		temp |= (v) >> (off ? (32 - off) : 0); \
...
#define ipu_ch_param_read_field_io(base, w, bit, size) ({ \
	...
	reg_offset = sizeof(struct ipu_ch_param_word) * (w) / 4; \
	reg_offset += i; \
	temp1 = io_read32((vaddr_t)(base) + reg_offset); \
	temp1 = mask & (temp1 >> off); \
	if (((bit)+(size)-1)/32 > i) { \
		reg_offset++; \
		temp2 = io_read32((vaddr_t)(base) + reg_offset); \
		temp2 &= mask >> (off ? (32 - off) : 0); \
		temp1 |= temp2 << (off ? (32 - off) : 0); \
	} \
	temp1; \
...
static inline void _ipu_ch_param_dump(struct ipu_soc *ipu, int ch)
{
	...
	DMSG("ch %d word 0 - %08X %08X %08X %08X %08X\n", ch,
		 p->word[0].data[0], p->word[0].data[1], p->word[0].data[2],
		 p->word[0].data[3], p->word[0].data[4]);
	DMSG("ch %d word 1 - %08X %08X %08X %08X %08X\n", ch,
	...
	DMSG("EBA0 0x%x\n",
		 ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 1, 0, 29) << 3);
	DMSG("EBA1 0x%x\n",
		 ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 1, 29, 29) << 3);
	DMSG("Stride %d\n",
		 ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 1, 102, 14));
	DMSG("scan_order %d\n",
		 ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 0, 113, 1));
	DMSG("uv_stride %d\n",
		 ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 1, 128, 14));
	DMSG("u_offset 0x%x\n",
		 ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 0, 46, 22) << 3);
	DMSG("v_offset 0x%x\n",
		 ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 0, 68, 22) << 3);
...
{
	int i, w;
	vaddr_t addr = (vaddr_t)ipu_ch_param_addr(ipu, ch);

	/* 2 words, 5 valid data */
	for (w = 0; w < 2; w++) {
		for (i = 0; i < 5; i++) {
			io_write32(addr, params->word[w].data[i]);
			addr += 4;
		}
		addr += 12;
	}
				      ...
				      dma_addr_t addr1, dma_addr_t addr2)
{
	...
	if (addr0%8)
		IMSG("IDMAC%d's EBA0 is not 8-byte aligned\n", ch);
	if (addr1%8)
		IMSG("IDMAC%d's EBA1 is not 8-byte aligned\n", ch);
	...
	case IPU_PIX_FMT_NV12:
		...
		break;
	default:
		EMSG("mxc ipu: unimplemented pixel format\n");
		break;
	}
	...
	DMSG("initializing idma ch %d @ %p\n", ch, ipu_ch_param_addr(ipu, ch));
	fill_cpmem(ipu, ch, &params);
	
	DMSG("channel param filled successfully.");
	// if (addr2) {
	...
	// 	DMSG("initializing idma ch %d @ %p sub cpmem\n", ch,
						...
						uint16_t burst_pixels)
{
	...
	DMSG("ipu_ch_20_param_addr: 0x%08x", ipu_ch_param_addr(ipu, ch));
	DMSG("burst size: %d", burst_pixels);

	/* originally with _io */
	ipu_ch_param_mod_field(ipu_ch_param_addr(ipu, ch), 1, 78, 7,
			       burst_pixels - 1);
	
	DMSG("ipu_ch_param_mod_field() succeed.");
...
static inline void _ipu_ch_param_set_alpha_buffer_memory(struct ipu_soc *ipu, uint32_t ch)
{
	...
	case 27: /* DP FG SYNC graphic */
		...
		break;
	default:
		EMSG("unsupported correlative channel of local "
			"alpha channel\n");
		return;
	}
...
static inline void _ipu_ch_param_set_axi_id(struct ipu_soc *ipu, uint32_t ch, uint32_t id)
{
	...
	ipu_ch_param_mod_field(ipu_ch_param_addr(ipu, ch), 1, 93, 2, id);
...
{
	return ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 1, 93, 2);
}
					 ...
					 uint32_t horizontal_offset)
{
	...
	old_offset = ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 0, 46, 22);
	if (old_offset != u_offset / 8)
		ipu_ch_param_mod_field_io(ipu_ch_param_addr(ipu, ch), 0, 46, 22, u_offset / 8);
	old_offset = ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 0, 68, 22);
			...
			uint32_t ch, uint32_t band_height)
{
	...
	DMSG("BNDM 0x%x, ",
		 ipu_ch_param_read_field(ipu_ch_param_addr(ipu, ch), 0, 114, 3));
}
